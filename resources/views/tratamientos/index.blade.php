...
@section('contenido')
    <h2 class="page-title" style="margin-bottom: 30px;">Gestión de Tratamientos</h2>
                ...
                @forelse($servicios as $servicio)
                    <tr style="border-bottom: 1px solid #eee;">
                        ...
                        <td style="padding: 15px; color: #888; font-size: 0.9em;">
                            <span
                                style="background:#f3f4f6; padding:4px 10px; border-radius:10px;">{{ $servicio->categoria ?? 'General' }}</span>
                        </td>
                        <td style="padding: 15px; color: var(--primary-color); font-weight: bold;">
                            ${{ number_format($servicio->precio_base, 2) }}</td>
                        <td style="padding: 15px; text-align: right;">
                            <button onclick="editarTratamiento({{ $servicio }})"
                                style="background: none; border: none; cursor: pointer; color: #f59e0b; margin-right: 10px;">
                                <i class="fa-solid fa-pen"></i>
                            </button>
                            <form action="{{ route('tratamientos.destroy', $servicio->id_servicio) }}" method="POST"
